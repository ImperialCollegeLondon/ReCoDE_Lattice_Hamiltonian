...
def plot_energies(
    ...
    ax.legend(
        ...
        title=input_parameter,
        title_fontsize=14,
    )

    if save:
        plt.savefig(
            save_path + f"Energy-vs-reh-Vary-{input_parameter}.png",
            bbox_inches="tight",
            dpi=300,
        )
...
def plot_state_distribution(
    L0, state_num: int, save: bool, sort_by_energy: bool = True, save_path: str = " "
):
    ...
    electron_density = np.resize(
        electron_density, (int(num_states**0.25), int(num_states**0.25))
    )
    hole_density = np.resize(
        hole_density, (int(num_states**0.25), int(num_states**0.25))
    )

    fig, axes = plt.subplots(1, 2, facecolor="white", figsize=(10, 10), dpi=600)
    sns.heatmap(
        electron_density.transpose(),
        cmap="Blues",
        robust=True,
        annot=False,
        cbar=False,
