...
def laguerre(alpha:int, n:int, x:float):
    L_0 = 1
    L_1 = 1 + alpha - x
    if n == 0:
        return L_0
...
def FCWD_single_nm_v2(n:int, m:int, lambda_inner:float, e_peak:float, lambda_outer:float, w:float):
...
def calc_FCWD_total(lambda_inner:float, e_peak:float, lambda_outer:float, w:float, N = 20, M = 6):
    kT = 0.0257 
    FCWD_total = 0
...
def decay_rate(lambda_inner:float, e_peak:float, lambda_outer:float, w:float, v:float):
