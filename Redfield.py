...
def J_Renger(w:float, lambda_total:float, e_peak:float, kT:float = 0.0257, n:int = 15):
    a = e_peak/(3/n)**(1/n)
    J_w = lambda_total*w**3*np.exp((-w/a)**n)
    J_w[w < 0] = 0
    return J_w
   
def norm_J_Renger(lambda_total:float, e_peak:float, n:int = 15):
    def func(w, a, n):
